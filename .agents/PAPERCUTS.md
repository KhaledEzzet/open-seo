...
- [ ] `2026-07-22T21:27:58Z` — `codex` — The manual Wrangler self-hosting guide tells users to replace the template KV/D1 resource IDs, while the adjacent `wrangler.jsonc` comment says changing those IDs orphans local data and to leave them unchanged. Clarify that manual deployers should replace the existing binding values (not append bindings) and scope the preservation warning to established local/self-hosted installations.
